...
#if QT_VERSION <                                                               \
    QT_VERSION_CHECK(5, 0, 0) // before Qt5, default pens used to be cosmetic if
...
QCPLayoutElement::QCPLayoutElement(QCustomPlot *parentPlot)
    : QCPLayerable(
      ...
      mMaximumSize(QWIDGETSIZE_MAX, QWIDGETSIZE_MAX), mRect(0, 0, 0, 0),
      ...
      foreach (QCP::MarginSide side, allMarginSides) {
                ...
                side)) // this side's margin shall be calculated automatically
        {
          if (mMarginGroups.contains(side))
            ...
            QCP::setMarginValue(
                newMargins, side,
                calculateAutoMargin(side)); // this side is not part of a group,
...
QCPAxis::QCPAxis(QCPAxisRect *parent, AxisType type)
    : QCPLayerable(parent->parentPlot(), QString(), parent),
      ...
      // tick labels:
      mTickLabels(true), mAutoTickLabels(true), mTickLabelType(ltNumber),
      ...
      mDateTimeSpec(Qt::LocalTime), mNumberPrecision(6), mNumberFormatChar('g'),
      mNumberBeautifulPowers(true),
      // ticks and subticks:
      mTicks(true), mTickStep(1), mSubTickCount(4), mAutoTickCount(6),
      mAutoTicks(true), mAutoTickStep(true), mAutoSubTicks(true),
      ...
      mScaleLogBase(10), mScaleLogBaseLogInv(1.0 / qLn(mScaleLogBase)),
      // internal members:
      mGrid(new QCPGrid(this)),
      mAxisPainter(new QCPAxisPainterPrivate(parent->parentPlot())),
      mLowestVisibleTick(0), mHighestVisibleTick(-1), mCachedMarginValid(false),
      mCachedMargin(0) {
  mGrid->setVisible(false);
  setAntialiased(false);
  setLayer(mParentPlot->currentLayer()); // it's actually on that layer already,
...
QCPAxisPainterPrivate::QCPAxisPainterPrivate(QCustomPlot *parentPlot)
    : type(QCPAxis::atLeft),
      ...
      mParentPlot(parentPlot), mLabelCache(16) // cache at most 16 (tick) labels
{}
...
QCPAbstractPlottable::QCPAbstractPlottable(QCPAxis *keyAxis, QCPAxis *valueAxis)
    : QCPLayerable(keyAxis->parentPlot(), QString(), keyAxis->axisRect()),
      ...
      mAntialiasedErrorBars(false), mPen(Qt::black), mSelectedPen(Qt::black),
      mBrush(Qt::NoBrush), mSelectedBrush(Qt::NoBrush), mKeyAxis(keyAxis),
      mValueAxis(valueAxis), mSelectable(true), mSelected(false) {
...
QCustomPlot::QCustomPlot(QWidget *parent)
    : QWidget(parent), xAxis(0), yAxis(0), xAxis2(0), yAxis2(0), legend(0),
      ...
      mMouseEventElement(0), mReplotting(false) {
...
*/
QCPData::QCPData()
    : key(0), value(0), keyErrorPlus(0), keyErrorMinus(0), valueErrorPlus(0),
      valueErrorMinus(0) {}
...
QCPData::QCPData(double key, double value)
    : key(key), value(value), keyErrorPlus(0), keyErrorMinus(0),
      valueErrorPlus(0), valueErrorMinus(0) {}
...
void QCPGraph::getPreparedData(QVector<QCPData> *lineData,
                             ...
                             // least two points per pixel on average
  {
    ...
    if (scatterData) {
      ...
      int reversedFactor =
          keyAxis->rangeReversed() ? -1 : 1; // is used to calculate keyEpsilon
                                             // pixel into the correct direction
      int reversedRound =
...
*/
const QPolygonF
QCPGraph::getChannelFillPolygon(const QVector<QPointF> *lineData) const {
  if (!mChannelFillGraph)
    return QPolygonF();
...
QPointF QCPCurve::getOptimizedPoint(int otherRegion, double otherKey,
  ...
  case 1: // top and left edge
  {
    ...
    if (intersectKey < rectLeft ||
        intersectKey > rectRight) // doesn't intersect, so must intersect other:
    {
  ...
  if (index >= 0) {
               ...
               (baseBars.size() - 1) / 2.0) // bar is to the left of center
    {
      ...
      } else           // uneven number of bars
      {
        ...
        result -=
            qAbs(upperPixelWidth - lowerPixelWidth) * 0.5; // half of center bar
...
QCPBars::QCPBars(QCPAxis *keyAxis, QCPAxis *valueAxis)
    : QCPAbstractPlottable(keyAxis, valueAxis), mData(new QCPBarDataMap),
      mWidth(0.75), mWidthType(wtPlotCoords), mBarsGroup(0), mBaseValue(0) {
...
QCPStatisticalBox::QCPStatisticalBox(QCPAxis *keyAxis, QCPAxis *valueAxis)
    : QCPAbstractPlottable(keyAxis, valueAxis), mKey(0), mMinimum(0),
      mLowerQuartile(0), mMedian(0), mUpperQuartile(0), mMaximum(0) {
...
QCPColorMapData::QCPColorMapData(int keySize, int valueSize,
    ...
    : mKeySize(0), mValueSize(0), mKeyRange(keyRange), mValueRange(valueRange),
      mIsEmpty(true), mData(0), mDataModified(true) {
...
QCPColorMapData::QCPColorMapData(const QCPColorMapData &other)
    : mKeySize(0), mValueSize(0), mIsEmpty(true), mData(0),
      mDataModified(true) {
  *this = other;
}
...
QCPColorMap::QCPColorMap(QCPAxis *keyAxis, QCPAxis *valueAxis)
    : QCPAbstractPlottable(keyAxis, valueAxis),
      ...
      mInterpolate(true), mTightBoundary(false), mMapImageInvalidated(true) {}
...
QCPFinancial::QCPFinancial(QCPAxis *keyAxis, QCPAxis *valueAxis)
    : QCPAbstractPlottable(keyAxis, valueAxis), mData(0), mChartStyle(csOhlc),
      mWidth(0.5), mTwoColored(false),
...
/* inherits documentation from base class */
QCPRange
QCPFinancial::getKeyRange(bool &foundRange,
    ...
    for (QCPFinancialDataMap::const_iterator it = begin; it != end; ++it) {
      ...
      double keyWidthPixels =
          ...
          keyAxis->coordToPixel(
              it.value().key - mWidth * 0.5); // sign of this makes sure
...
QCPItemTracer::QCPItemTracer(QCustomPlot *parentPlot)
    : QCPAbstractItem(parentPlot),
      position(createPosition(QLatin1String("position"))), mGraph(0) {
  position->setCoords(0, 0);
